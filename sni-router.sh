#!/bin/bash
# ╔══════════════════════════════════════════════════════════════════╗
# ║        SNI + HOST Port Multiplexer  v2.2.17 — by acrnm          ║
...
GLOBAL
    ...
    mv "$tmpfile" "$HAPROXY_CFG" && trap - RETURN
}
...
action_toggle_port() {
    ...
    if [[ "$cur" == "1" ]]; then
        ...
        if [[ "$ok" == "y" || "$ok" == "Y" ]]; then
            state_set "$key" "0"
            apply_config && info "Port ${pt} disabled — HAProxy no longer listens on it." || { err "Reload failed"; state_set "$key" "1"; apply_config || true; }
        else
            warn "Cancelled."
        fi
    else
